...
"$PY_KOMUT" - << 'PY' 2>/dev/null && kontrol "Butun kancalar settings.json'da kayitli" 0 || kontrol "Butun kancalar settings.json'da kayitli" 1
import json
...
"$PY_KOMUT" - << 'PY' 2>/dev/null && kontrol "Kurulum betigi butun kancalari taniyor" 0 || kontrol "Kurulum betigi butun kancalari taniyor" 1
import ast, sys
...
"$PY_KOMUT" "$P/scripts/testler/koruma-testleri.py" > _calisma/koruma-sonuc.txt 2>&1
...
"$PY_KOMUT" -c "import json;json.load(open('$P/references/sunucu-haritasi.json',encoding='utf-8'))" 2>/dev/null \
  && kontrol "Sunucu haritasi gecerli JSON" 0 || kontrol "Sunucu haritasi gecerli JSON" 1

"$PY_KOMUT" - << 'PY' 2>/dev/null && kontrol "Haritada parola/anahtar YOK" 0 || kontrol "Haritada parola/anahtar YOK" 1
import json, re
...
"$PY_KOMUT" "$P/scripts/kuru-deneme.py" "git reset --hard" 2>/dev/null | grep -q "ONAY İSTER" \
  && kontrol "Sanal deneme yikici komutu bildiriyor" 0 || kontrol "Sanal deneme yikici komutu bildiriyor" 1
"$PY_KOMUT" "$P/scripts/kuru-deneme.py" "ls -la" 2>/dev/null | grep -q "engel görmez" \
...
"$PY_KOMUT" - << 'PY' 2>/dev/null && kontrol "Gerekceler Turkce ve dort parcali" 0 || kontrol "Gerekceler Turkce ve dort parcali" 1
